...
export default async function handler(req, res) {
	...
	if (req.method === 'GET') {
		const Carts = [];
		if (session) {
			await User.findById(session.user.uid).populate('cart.items.productId');
		}
		if (readyData.cart) {
			...
			return res.status(200).json({ main: Carts[0], status: session ? 'MERGED_CARTS' : 'COOKIE_CART' });
		} else {
			if (session) {
				return res.status(200).json({ main: Carts[0], status: 'SESSION_CART' });
			} else {
