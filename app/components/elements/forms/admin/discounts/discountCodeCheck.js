...
export default function DiscountCodeForm({ cart }) {
	...
	return cartData.discount ? (
		<Alert variant="info">
			Do zamówienia przypisany jest kod zniżkowy <strong>{cartData.discount.code}</strong>
		</Alert>
	) : (
		<Formik
		...
		>
			{({ errors, values, touched, handleChange, handleSubmit, validateField, setFieldValue }) => (
				<Form id="addDiscountToCart" onSubmit={handleSubmit}>
					<Form.Label>Jeżeli taki posiadasz, możesz podać kod zniżkowy poniżej:</Form.Label>
