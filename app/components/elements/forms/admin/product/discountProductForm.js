...
export default function DiscountProductForm({ productData }) {
	...
	return (
		<Formik
			...
			initialValues={{
				discountValue: productData.discount.discountValue ? productData.discount.discountValue : 0,
				discountUntil: productData.discount.discountUntil
					? dayjs(productData.discount.discountUntil).format('YYYY-MM-DD')
					: dayjs().format('YYYY-MM-DD'),
			}}
		>
			{({ errors, values, touched, handleChange, handleSubmit, submitForm, setFieldValue }) => (
				<Form onSubmit={handleSubmit} id="editProductForm">
					...
					<Form.Group className="mt-2" controlId="discountUntil">
						...
						<Form.Control
							...
							onChange={handleChange}
						/>
						<Form.Control.Feedback type="invalid">{errors.discountUntil}</Form.Control.Feedback>
