...
function NewClientForm(props) {
	const schema = yup
		...
		.shape({
			...
			password_r: yup.string().oneOf([yup.ref('password'), null], 'hasla musza byc takie same'),
		})
		.required();
	...
	return (
		<Form onSubmit={handleSubmit(onSubmit)} className="col-6">
			<Form.Group>
				...
				<Form.Control type="text" id="name" {...register('name')} isInvalid={!!errors.name} />
				<Form.Control.Feedback type="invalid">{errors.name?.message}</Form.Control.Feedback>
			</Form.Group>
			<Form.Group>
				<Form.Label htmlFor="email">Email</Form.Label>
				<Form.Control type="text" id="email" {...register('email')} isInvalid={!!errors.email} />
			...
			<Form.Group>
				...
				<Form.Control type="password" id="password_r" {...register('password_r')} isInvalid={!!errors.password_r} />
				<Form.Control.Feedback type="invalid">{errors.password_r?.message}</Form.Control.Feedback>
			</Form.Group>
			<div className="pt-2">
